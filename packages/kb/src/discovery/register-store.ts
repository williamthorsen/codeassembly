...
export async function registerStore(input: {
  ...
  // Write the entry's own keys in alphabetical order, matching how the registry's entries are ordered.
 ...
 * preserves the document: A comment attached to an entry moves with it, and a comment preceding the first entry belongs
 * to the block itself and stays at its head.
 */
...
/**
 * Reads a registry entry's name from its key. A key parsed from the file is a scalar node, while one just written by
 * `setIn` is stored as a raw string, so both forms have to be read.
 */
