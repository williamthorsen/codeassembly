...
const FENCE = '---';
// Stringify options for single string scalars.
// `schema: 'core'` matches the parser in `parse-note.ts`, so the library quotes exactly the strings that would
// otherwise re-parse as a non-string (numbers, the special floats `.inf`/`.nan`, booleans, `null`/`~`).
// `defaultStringType: 'PLAIN'` keeps round-trip-safe strings unquoted;
// `singleQuote: true` selects single quotes when quoting is unavoidable.
...
/**
 * Renders a string scalar, delegating the quoting decision to the `yaml` library's `core`-schema stringifier so the
 * result re-parses to the same string. The library quotes only when a plain scalar would round-trip to a non-string —
 * covering YAML core integers, floats (including `.inf`/`.nan`), booleans, and `null`/`~` — and leaves safe strings
 * unquoted. A value containing a newline yields a multi-line block scalar, which cannot occupy a single `key: value`
 * line; such values are re-rendered as a double-quoted scalar, whose `\n` escapes keep the value on one line and
 * round-trip the embedded newlines faithfully.
 */
