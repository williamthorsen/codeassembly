...
/** A temporary git repository into which a staleness test commits. */
export interface GuidanceRepoFixture {
  /** Repository root, containing everything the fixture created. */
  root: string;
...
/**
 * Builds an empty temporary git repository with an identity of its own, so that the commits made by a test
 * need neither the developer's git config nor a writable global one.
 */
