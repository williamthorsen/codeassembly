...
/**
 * Builds an event store and an isolated home registering it, then returns an invocation that scopes to a
 * store name that the registry does not list. Exercises the bundled resolver from home discovery through
 * registry parse to the scope verdict and the JSON result shape, stopping short of recall.
 *
 * Keep the invocation off the recall path: Recalling here would put ripgrep on the critical path of every
 * build.
 */
...
/** Asserts the kb-retrieve-events smoke resolved the registry and reported the requested store as unregistered. */
