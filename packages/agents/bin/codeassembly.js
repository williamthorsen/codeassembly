...
// Thin wrapper so that pnpm can symlink the bin at install time, before `dist/`
...
if (!existsSync(entryPoint)) {
  process.stderr.write(
    'codeassembly: build output not found. In a source checkout, run `pnpm run bootstrap`; otherwise reinstall the package.\n',
  );
  process.exit(1);
}
...
try {
  ...
  process.stderr.write(`codeassembly: failed to load: ${message}\n`);
  process.exit(1);
}
