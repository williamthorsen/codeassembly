...
**Provider.** A repo that produces its own artifacts syncs after the build that produces them, and fails on error: the build has already succeeded by then, so the tree is usable and a non-zero exit is the signal rather than a broken checkout. This trigger cannot move to `postinstall`, because pre-build content is incomplete and the `codeassembly-agents` wrapper exits before it parses `--warn-only` when the build output is absent.
