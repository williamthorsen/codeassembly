...
No whitespace pass runs after substitution, so each group carries its own separators — `[{ticket_ref} ]{title}`, not `[{ticket_ref}] {title}`. That is what lets `describe-change.mjs --parse` read a rendered title back into the record that produced it, and what lets `--classify` read a whole commit range back through `commit.title_format`. See [title-templates.md](content/skills/_data/title-templates.md) for both modes, for `--record-block`, which renders the `change-record` block that ends a pull-request body, and for `--resolve-merge`, which resolves what a pull request merges as from that block and its commits. See [change-record.md](content/skills/_data/change-record.md) for how a derived classification is written down and read back.
...
A `rule` is any lowercase kebab-case name a bound rulebook declares, detected or not. `plain-speech` carries no detector and is recorded like any other, which is what lets a unit hold judgments without owning a detector.
