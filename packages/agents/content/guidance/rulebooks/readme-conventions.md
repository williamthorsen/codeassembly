...
| Library or package (`library`)                | The import line and a minimal use in about three lines      | What problem it solves, and how it compares to known alternatives  | A full API listing, once it outgrows the first screen |
...
| Monorepo root (`monorepo-root`)               | What the repository is, and a map of its packages           | One bootstrap command, and where each package's README is          | Anything a package's own README states                |
...
Detail about one symbol's behavior, such as its edge cases, its failure modes, and the reason for a rule that it enforces, belongs in that symbol's doc comment, where a caller reads it while using the symbol. A README paragraph that would make a good doc comment is a doc comment in the wrong file.
...
- Badge walls and marketing copy. A badge that a reader acts on justifies its line; the rest is decoration.
