...
A support entry under `skills/` contributes no edge: It is reached by a link rather than inlined, so `sync` never reads it, and a `{skill:<slug>}` or `{subagent:<slug>}` token in one renders a pointer that nothing pulls into the install. Each skill or subagent that links into the section carrying such a token declares its target under `dependencies:`. The requirement follows the fragment, so a bare link to the file carries none, and a link to an enclosing heading carries the same requirement as one to the subsection itself. _(Enforced by test: `support-entry-tokens.unit.test.ts`, which also fails a support-entry token naming an artifact the library does not contain.)_
