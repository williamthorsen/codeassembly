...
A skill or subagent token has an optional form, marked `?` before the colon: `{skill?:<slug>}` and `{subagent?:<slug>}`. It renders exactly as the required form does, so an invocation reads the same wherever the target deploys, and its target must still exist -- `sync` resolves one, so a rename or a deletion fails the run. What it does not do is contribute an edge, so the target deploys only where a declaration reaches it another way. Reach for it where a body names an artifact a consumer may not want, such as a platform delegate for an ecosystem the consumer does not use.

`{rulebook?:<slug>}` fails the run: A rulebook token renders the skill name its target deploys under, and an undeployed target supplies that name nowhere. `dependencies:` has no optional counterpart, because a frontmatter entry exists to pull an artifact in; a body that names one without pulling it in is the only case the marker answers. A content root carrying an optional token declares content format 2, in any body it ships, a `_partials` file included. _(Enforced by test: `content-format-declaration.unit.test.ts`, and reported by `validate` for a consumer's own root.)_

A support entry under `skills/` contributes no edge: It is reached by a link rather than inlined, so `sync` never reads it. Each skill or subagent that links into the section carrying a required `{skill:<slug>}` or `{subagent:<slug>}` token declares its target under `dependencies:`. An optional token there is exempt, since asserting the target may be absent is the opposite of guaranteeing its presence. The requirement follows the fragment: A bare link to the file carries none, and a link to an enclosing heading carries its subsections'. _(Enforced by test: `support-entry-tokens.unit.test.ts`, which also fails a support-entry token naming an artifact the library does not contain, optional or required.)_
...
- A consumer declares it only where that vendor's products are in use, since each member costs a line in the skill index of every session.
...
Behavioral rules for an agent's output -- such as the recommendation gradient and the action-items block -- are stated once in `AGENTS.md` and the shared `_data` specs. Where the boundary below requires a restatement, put it at the step that produces the output: as a pointer in the skill body, or as a rendered example inlined from `_partials/`. An agent follows a rule more reliably when the rule appears next to the action to which it applies than when the agent must follow a link to read it, and it imitates a nearby concrete example more reliably still than it follows a directive.
