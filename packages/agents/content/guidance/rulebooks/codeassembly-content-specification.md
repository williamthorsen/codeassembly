...
A support entry under `skills/` contributes no edge: It is reached by a link rather than inlined, so `sync` never reads it. Each skill or subagent that links into the section carrying a `{skill:<slug>}` or `{subagent:<slug>}` token declares its target under `dependencies:`. The requirement follows the fragment: A bare link to the file carries none, and a link to an enclosing heading carries its subsections'. _(Enforced by test: `support-entry-tokens.unit.test.ts`, which also fails a support-entry token naming an artifact the library does not contain.)_
