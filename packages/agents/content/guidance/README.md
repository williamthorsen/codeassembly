...
**Destination tiers.** Harness-neutral and per-harness destinations are not interchangeable. `~/.agents/AGENTS.md` (rendered from `shared/`) and `.agents/PROJECT.md` are neutral — one file serves every harness — so neither can carry a harness-scoped path, and content that needs one belongs in an ambient rulebook instead. Every generated destination now has a known target harness: skills, subagents, and both ambient tiers. `.agents/PROJECT.md` no longer receives generated content at all; it is hand-authored, and `sync` strips the rulebook blocks a previous version left there.
