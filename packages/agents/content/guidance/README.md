...
- **Project scope** (`sync`): the host is each targeted harness's machine-local project guidance file at the project root (`CLAUDE.local.md`, `AGENTS.local.md`). `install` does not manage user-local files, so `sync` owns region creation here: an absent host is created carrying the region, and a host that already exists keeps everything it holds and gains the region below it. A host is materialized only when the scope declares an ambient rulebook, so a project that declares none is left with no file; and a host carrying a half-written region fails the run rather than being appended to, since appending beside a stray marker would swallow the text between them on the following sync. Because the host is gitignored, it exists only in the worktree that created it, so a multi-worktree checkout needs a sync per worktree; `sync` warns when a host it writes is not git-ignored.

**Destination tiers.** Harness-neutral and per-harness destinations are not interchangeable. `~/.agents/AGENTS.md` (rendered from `shared/`) and a project's root `AGENTS.md` are neutral: one body of text serves every harness, so neither can carry a harness-scoped path, and content that needs one belongs in an ambient rulebook instead. Every generated destination has a known target harness: skills, subagents, and both ambient tiers. The root `AGENTS.md` receives no generated content at all. It is hand-authored, and `sync` strips the rulebook blocks a previous version left there or in the `.agents/PROJECT.md` it superseded.
