...
// ships as a rendered sigil that the agent follows to nothing. Resolving them against the catalog turns that into a
// failure.
//
...
/** A host that links into a support entry, and the sections of it that the host addresses. */
...
/** One link from a host into a support entry: the entry's path, and the heading slug named by the fragment. */
...
describe('support entry invocation tokens', () => {
  it('name artifacts that the library contains', async () => {
    ...
    for (const file of files) {
      ...
      // This assertion covers optional targets as well as the required ones: The closure walk never reads a support
      // entry, so nothing else catches a slug that no longer names an artifact, whichever form the token takes. Each
      // slug keeps its form. The reported token is the string that the author finds in the file.
    ...
    const message =
      'A support entry carries an invocation token naming an artifact missing from the library. Nothing else ' +
...
describe('support entry token declarations', () => {
  it('are declared by every host that links into the section carrying them', async () => {
    ...
    const message =
      ...
      'reaches, so the entry ships a pointer to an artifact that the consumer does not install. Declare the target ' +
 ...
 * section linked by the host. One walk serves the library and the fixtures alike, so what the fixtures prove about
 * the detection is what the library is held to.
 */
 ...
 * that runs until the next heading of its level or shallower, so the open headings at any point are the chain that a
 * link fragment can address to reach that point.
 */
...
/**
 * Lists every skill and subagent whose include-expanded body links into a support entry, with the sections that it
 * names.
 */
...
/**
 * Maps each support entry to the artifacts named by its required invocation tokens, keyed by every heading slug whose
 ...
 * be absent, which is the opposite of what the host declaration that it would compel guarantees.
 */
