...
// relative target against the *host* file's directory (the skill or subagent into which the link renders, not the
// partial in which it may have been authored) and leaves an anchor-only target untouched. A link whose target has
// moved or been deleted still installs cleanly, leaving the agent to follow a link to nothing. Resolving every link
// the way the installer does turns that into a build failure.
//
// Both halves of a link are checked: The file must exist, and any `#fragment` must name exactly one heading in the
// file into which it points. Skills reach their inlined output-shaping specs through in-file anchors, so an
// unvalidated fragment is a dead locator repeated across every consumer.
//
// A host that leaves a code fence open is reported before either check runs. Everything below such a fence is read as
// code. The links and headings that the host appears to contain are not the ones that it does, and a clean result
// over it would be indistinguishable from a checked one. The render gate throws on the same condition.
//
// The render pass rejects a same-body anchor on its own, over content from any source. This suite still covers it,
// because it reports every violation across the tree at once whereas the render pass throws on the first artifact,
// and because a fragment on a cross-file target is checked nowhere else: The deployed tree unions library content
// with each declared source's, so the render pass cannot resolve one from the content root at hand. Both share one
// slug algorithm through `anchor-resolution`.
//
...
// through each host, which is the only context in which they mean anything.
//
...
describe('installable-content link resolution', () => {
  ...
  it('every anchor fragment resolves to exactly one heading in the file into which it points', () => {
 ...
 * undeliverable link target or an unusable `{rulebook:<slug>}` token. Every shipped rulebook stands in for the
 * deployed set, which is the strictest catalog available here: A token naming one that is missing or ambient-only has
 * nothing to invoke under any declaration. Because the root allowlist and the catalog are both harness-invariant,
 * one harness context stands for all.
 */
...
async function findViolations(): Promise<ReadonlyArray<Violation>> {
  ...
  for (const hostFile of hostFiles) {
    ...
    // Everything below an open fence is blanked. The anchors and links that this host appears to contain are not the
    // ones that it does. Reporting the fence and moving on matches the order that the render gate uses for the same
    // reason.
...
function formatViolations(violations: ReadonlyArray<Violation>): string {
  ...
  const header =
    ...
    `file into which it points. A link authored in a partial is reported against every host that inlines it, so ` +
    `fix the partial rather than the host.`;
