...
- **The drive-by doesn't mix concerns within a single file.** The standard "drive-by changes are bad" heuristic triggers too often: The real cost arises when concerns mix in one file. **Changes in completely unrelated files are good drive-by candidates**: The reviewer can skim past them.
...
Walk back through the list and apply the rule: Any item that fails drive-by suitability moves to batch; any item that doesn't fit a batch becomes a separate ticket.
