...
The renderer is Python-Markdown, which is the source of the 4-space requirement: The divergence from GFM is a dialect difference rather than a defect in this client. The tool round-trips the submitted Markdown byte for byte in `description.raw`, so when a body reads correctly there and renders wrong in the browser, the fault is in the renderer.

The same renderer applies to a merge commit message, since Bitbucket's commit list renders a commit message as Markdown. [BCLOUD-19083](https://jira.atlassian.com/browse/BCLOUD-19083) records that behavior, asking for a way to switch it off after Bitbucket displayed a `#`-prefixed commit title as a heading.

## Reading a pull request

`action: "get"` with `prId` returns the Bitbucket REST pull-request object verbatim. The fields read by these skills:
...
Body text comes from `description` and nowhere else. `rendered.description` contains the same content as HTML, and `summary` contains a duplicate raw copy; a skill that substitutes either for `description` publishes or reviews the wrong text.

**`source.commit.hash` is not a fixed width.** Bitbucket has been observed returning it abbreviated to 12 characters, whereas `git rev-parse HEAD` returns 40. Compare on a prefix rather than on equality or on a fixed truncation: The platform's hash matches when it is a non-empty prefix of the local SHA and is at least 7 characters long. A shorter or empty hash fails the comparison rather than matching everything.
...
Bitbucket Cloud offers six strategies. The three that a caller needs:
