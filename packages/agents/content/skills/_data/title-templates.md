...
Each field names the step that set it last, as [Where the record is read](./change-record.md#where-the-record-is-read) orders the steps, so a field that an override sets names the override even when it repeats the value that it replaces. A field is `null` when nothing supplied it: a `ticket_ref` that neither the title nor `--ticket-ref` names, and, without entries to rank, a field that the labels do not resolve while the commits cannot be read.
...
- A change to root and exactly one workspace takes that workspace, since the root files support its work.
- Use `*` when the change spans multiple workspaces, with or without root. It normalizes to no scope, so the rendered title has no scope prefix.
...
**Consolidation counts each named workspace separately.** One entry naming `agents,kb` contributes both, exactly as two entries naming one apiece do, so it consolidates to no scope. Consolidation first sets aside the scopes of entries in the taxonomy's last tier (`process` in the bundled taxonomy) whenever an entry of a higher tier names a scope, because process work such as a catalog move supports the branch's change rather than describing it: A `feat` scoped `img-promoter` and a `deps` scoped `web` consolidate to `img-promoter`. It then sets `root` aside whenever the remaining scopes also name a workspace, and keeps a scope only when exactly one remains: `agents` and `root` consolidate to `agents`, and `root` alone to `root`. The scope labels omit `root` beside a workspace in the same way but keep a process-tier entry's workspace, since a label names every workspace that the branch changed.
