...
**Generalize the list, not the change.** When the facts outgrow the budget, replace the list with what the change amounts to -- the upshot, never a category of the parts: "Regroups `readyup`'s source one level below `src/`", not "Reorganizes several utility modules". Spend what is left on the noteworthy fact, the one a reader given the upshot could not have predicted: the regroup's spare sentence went to a new lint rule banning `utils/` directories, not to the rewritten import paths every regroup entails. Noteworthiness, not size, is the test: an inferable clause is cut first, however small. The floor is "Cut detail, not meaning" below: an upshot the reader cannot restate the change from has been abstracted past its job.
