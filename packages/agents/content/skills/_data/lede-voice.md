...
**Mechanism is substance.** For a developer reader, the operation performed -- the rename, the upgrade, the extraction, the new check -- is exactly what they want to know. Naming it is not implementation detail; it is the news. At the release-notes register, mechanism earns its place when it explains the visible change. How much of it fits depends on how much room the lede has: on a small change, the mechanism of every part; on a large one, the mechanism of the surviving fact and no more. Where the operation is itself what the change amounts to, as in a `refactor` or an `internal` restructuring, it is the surviving fact rather than a detail competing with one.
...
**Punch the highlights.** The budget is one to three sentences, and it does not grow with the change: a diff touching nine files gets what a one-line fix gets. The question is never whether a fact is important enough to keep, since every true fact in a diff clears that bar, and clearing it one fact at a time is how a lede becomes a catalog. The question is what these three sentences are spent on, and which fact leads. Nothing is lost by spending them elsewhere: a fact cut from the lede survives in `## Details`, in the body of the changelog entry, and in the diff, one click from any reader who wants it. A lede is a summary with a point of view, and craft is welcome: a vivid concrete detail ("earns a rocket emoji in the terminal output") informs better than an abstraction, and a correct but flat recitation is itself a failure.

**Generalize the list, not the change.** When the facts no longer fit, replace the list with what the change amounts to, then spend what is left on the fact that summary does not imply. What it amounts to is the upshot, never a category of the parts: a trip home with hamburgers, buns, extra ketchup, pickles, toothpicks, a blanket, and an ice pack bought "all the supplies for our hamburger picnic", not "groceries and picnic gear" -- and the sentence left over goes to the extra ketchup, which that summary does not imply, not the blanket, which it does. Size is not the test; predictability is, so a subordinate clause the reader could have inferred goes first however small it is. "Cut detail, not meaning" below is the floor: an upshot the reader cannot restate the change from has been abstracted past its job.
...
- Most ledes are one to three sentences, and a larger change does not earn more.
