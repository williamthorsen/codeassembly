...
**Mechanism is substance.** For a developer reader, the operation performed -- the rename, the upgrade, the extraction, the new check -- is exactly what they want to know. Naming it is not implementation detail; it is the news. At the release-notes register, mechanism earns its place when it explains the visible change. But mechanism is also the first detail the budget cuts ("Punch the highlights" below): a small change affords it for every part, a large one only for the fact its lede keeps. Where the operation is itself what the change amounts to, as in a `refactor` or an `internal` restructuring, mechanism is the summary rather than a detail competing with it.
...
**Punch the highlights.** The budget is one to three sentences, and it does not grow with the change: a diff touching nine files gets what a one-line fix gets. Never ask whether a fact is important enough to keep -- every true fact in a diff clears that bar, and a lede becomes a catalog one justified keep at a time. Ask what the three sentences are spent on and which fact leads; a cut fact is not lost but survives in `## Details` and the diff, one click from any reader who wants it. A lede is a summary with a point of view, and craft is welcome: a vivid concrete detail ("earns a rocket emoji in the terminal output") informs better than an abstraction, and a correct but flat recitation is itself a failure.

**Generalize the list, not the change.** When the facts outgrow the budget, replace the list with what the change amounts to -- the upshot, never a category of the parts: "Regroups `readyup`'s source one level below `src/`", not "Reorganizes several utility modules". Spend what is left on the noteworthy fact, the one a reader given the upshot could not have predicted: the regroup's spare sentence went to a new lint rule banning `utils/` directories, not to the rewritten import paths every regroup entails. Unpredictability, not size, is the test: an inferable clause is cut first, however small. The floor is "Cut detail, not meaning" below: an upshot the reader cannot restate the change from has been abstracted past its job.
...
- A lede is one to three sentences, and a larger change does not earn more; only a second concern or migration info earns a further paragraph, never size.
...
- A second concern gets its own short paragraph, often marked ("Separately, ...") and held to the same budget; a facet of the main change is not a second concern. Migration or breaking info that earns a paragraph gets a labeled one ("Migration: ..."). Three or more parallel items may be bulleted.
