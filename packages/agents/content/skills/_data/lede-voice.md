...
**The title is already on the page.** Every surface that renders a lede shows the change's title above it: the pull request's own title, the changelog entry's line, the merge commit's subject. The reader has read that title before reaching the first sentence, so a sentence restating it spends their opening seconds on what they already know. Report what the title does not. Where the title says all there is, the sentence below it still names the detail the title had no room for.
