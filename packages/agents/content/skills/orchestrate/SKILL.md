...
- **Recovery from reviewer interruption**: when a reviewer Task returns without a structured return block (typically an `agentId:` marker on `max_turns` exhaustion), the reviewer maintains its review file incrementally — read the partial artifact at the canonical reviewer path (`{run-dir}/{NN}_{reviewer}_*.md`). Inspect the `### Criticality:` line: if it is the literal sentinel `(pending)`, the reviewer did not converge. Treat the dispatch as `failed` for flow control purposes (per the `failed`-reviewer rule in `modules/review-cycle.md`'s "Handling failures" note, the reviewer's contribution to aggregated criticality is `medium`), but retain the partial findings list to inform any continuation dispatch and the run summary. Do NOT use `(pending)` as a criticality value in aggregation — it is not in the enum. Do NOT fall back to working-tree inspection; the partial artifact is the authoritative state-transfer channel. This recovery path applies to the four reviewers that return structured blocks: `orchestrated-reviewer`, `aspect-code-reviewer`, `aspect-silent-failure-reviewer`, `aspect-test-reviewer`. `code-simplification-reviewer` has no structured return block — its partial artifact is read directly by the existing simplifier flow in `modules/review-cycle.md`.
