...
Use `commit_title` as the commit title verbatim -- it already contains the rendered prefix, per the configured `commit.title_format`, and the bare title text. Where the script is not found, fall back to the bare `{title}`.
