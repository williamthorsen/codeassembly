...
The pipeline prints the rendered title. Read it from the command's output and use it verbatim as the commit title, which already contains the rendered prefix, per the configured `commit.title_format`, and the bare title text. Assigning the parse instead prints nothing, and no shell variable survives to the `git commit` call, so the title must be carried forward as literal text. Where the script is not found, fall back to the bare `{title}`.
