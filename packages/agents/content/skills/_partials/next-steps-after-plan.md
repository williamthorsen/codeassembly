...
**One `➕` line is mandatory rather than omitted.** When Refine plan is the selected option, it must include a `➕` line naming the specific unsettled decision the pass would surface (rule 1). The line names an open decision, never a reassurance about work already done: "a refine pass is the cheap way to find out whether I missed something" is the shape that this requirement exists to forbid. Selecting Refine plan without such a line is a defect: If the line cannot be written, rule 1 did not match and the cascade continues to rule 2.
