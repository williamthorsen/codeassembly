...
Example (rendered for the default case, in which the recommendation rules below select Orchestrate):
...
See [`scope-and-deferral.md`](../_data/scope-and-deferral.md) for the related decision on whether a finding warrants its own ticket. That decision (do now / batch later / separate ticket) composes with the recommendation rules above: The rules here pick the next-step _skill_; that reference decides whether work that comes up alongside the current plan should get its own ticket or be merged with the current change.
