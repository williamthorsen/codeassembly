...
**Resolve the scratch directory; never reference it.** Create one with `mktemp -d "${TMPDIR:-/tmp}/gh-body.XXXXXX"` in a Bash call and use the absolute path that call prints; bare `mktemp -d` picks a path denied by the agent sandbox. The {tool:Write} tool performs no shell expansion, so a path containing `$TMPDIR` handed to it creates a directory named `$TMPDIR`.
