...
Run the sweep in the main session, never in a subagent. Only the session contains the conversation the artifacts are swept against.
...
**Route each gap by kind**, per the placement doctrine: The change's subject and outcomes fold into the ticket, mechanism folds into the plan. Where the gap came up in conversation does not decide where it goes. When the invocation produces no ticket artifact, a subject-level gap folds into the plan's context section.

**Report and proceed.** When the sweep finds gaps, fold them in and report the amendments as a brief list, then save. The report is informational: The amendments record decisions the conversation already settled, so it opens no new approval gate and asks nothing. When the sweep finds nothing, save silently; a clean sweep is not worth a line.
