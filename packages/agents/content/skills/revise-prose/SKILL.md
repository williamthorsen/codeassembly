...
3. **Commit each batch in index order.** Stage that batch's files alone and commit them per `{skill:create-commit}`, with type `docs` and the scope that `create-commit` derives from those files. Where the files span workspaces, `describe-change.mjs` renders the prefix as `*|docs:`; correct it to a bare `docs:`.
