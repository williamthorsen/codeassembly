...
Both come from the text you are reading now, never from a list held elsewhere. A rule document opts into a detector by naming the rule, so a heading change cannot silently drop a rule, and a project bound to a different rulebook gets candidates only for the rules that it declares.
...
Dispatch batch 0 alone, per step 4's dispatch shape. When it returns, show the user its report and `git diff --stat`, then ask for a go-ahead before committing that batch and before dispatching anything else.
...
Before each dispatch, write that batch's candidate objects, exactly as the helper reported them and `stale` flags included, to `$TMPDIR/revise-prose/batch-{index}.json`.
...
candidates: $TMPDIR/revise-prose/batch-{index}.json
units: {name}={version}, {name}={version}
```
...
1. **Parse the report.** It is one fenced JSON block carrying `applied`, `rejected`, and `questionable`. A return that carries no such block, or one that is truncated, is a redispatch of that batch rather than an edit.
2. **Check the diff.** Run `git diff --name-only`. Where it names a file outside the batch, stop the run and report which file changed and which batch changed it; commit nothing.
3. **Commit the batch.** Stage the batch's files alone and commit them per `{skill:create-commit}`, with type `docs` and the scope that `create-commit` derives from those files. Where the files span workspaces, `describe-change.sh` renders the prefix as `*|docs:`; correct it to a bare `docs:`.
4. **Accumulate** the batch's `rejected` and `questionable` entries for step 5.
...
3. **Compose the fold** and pipe it to the helper's `record` command, which is the record's only write path:
   ...
   `sweptAt` is today's ISO calendar date. `units` names every unit from step 1 with the version it is at and its `roots`: the invocation's narrowing paths, or `["."]` for a whole-repository sweep. `rejections` holds every subagent rejection plus every questionable that the user rejected, each carrying `rule`, `unit`, `file`, `phrase` as the text reads after this run's edits, and `ground`. Drop a rejection whose unit the fold does not name, and say so in the summary.
