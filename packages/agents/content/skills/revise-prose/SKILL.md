...
- **Each `<!-- rule: <id> <version> -->` marker** in a bound rulebook's body names a rule to the helper, whether or not the helper has a detector for it. A marker that declares no version reads `<!-- rule: <id> -->`. Either way, name the rule to the helper by its id alone. The rule's unit is the block containing the marker.
