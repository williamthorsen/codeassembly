...
{This section becomes a changelog entry — and, for public-tier work (per [work-types.json](../_data/work-types.json)), a release note. Follow the [release-notes voice](../_data/release-notes-voice.md): Apply the per-sentence outcome test (outcomes, invariants, migration info — never mechanism) and the identifier ban (top-level user-configurable surface only). Mechanism, internal naming, and refactor mechanics belong in `## Details`.}
