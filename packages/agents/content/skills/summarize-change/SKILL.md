...
2. **Analyze changes**:
   ...
   - **Type the change from the diff.** Apply [Work type test](#work-type-test) to the diff and resolve the type from that, whatever the consolidated record and `ticket_type` name. Agreement between the two is not evidence: The commits are often typed from the ticket's label, so one misclassification reaches both. Where the test's type differs from the effective type, set `override_type` to it and re-run `resolve-effective-record`. Ask the developer only where the test leaves two types genuinely close, following [option format](#option-format): a clear result is a decision to state in one line with its reason, not a menu. Resolve here rather than later, since the lede's tier in step 5 follows the type. A session with no developer to ask records both types and asks nothing.
