...
- **An `any` or a type assertion suppresses in the language what a directive suppresses in a comment.** Both silence the checker without changing what the code does, so the same ladder applies to both: Model the type the value actually has, or return `unknown` and let the caller narrow it.
- **Write a rationale for every suppression you introduce**, naming the rule, why it doesn't apply here, and what alternatives you rejected. Restating the rule is not a rationale.
