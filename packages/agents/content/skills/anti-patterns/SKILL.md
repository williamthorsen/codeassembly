...
- **Try these in order before suppressing inline:** (1) change the code so that the rule no longer triggers, usually the right answer; (2) reconfigure the rule if its default is wrong for the project; (3) define a scoped exception for a whole category that is legitimately exempt. Suppress inline only as a last resort, when the case is genuinely local and none of the above fits, such as an external boundary where the real type can't be known.
- **An `any` or a type assertion suppresses in the language what a directive suppresses in a comment.** Both silence the checker without changing what the code does, so the same ladder applies to both: Model the type that the value actually has, or return `unknown` and let the caller narrow it.
- **Write a rationale for every suppression that you introduce**, naming the rule, why it doesn't apply here, and what alternatives you rejected. Restating the rule is not a rationale.
...
// ✅ Change the code so that the rule no longer triggers
function classify(order) {
