...
What would you like to do? Reply with numbers, or 'all'.
```
...
4. **Post insights to ticket**: For each `ticket comment` insight, write the insight body to a scratch file per [gh body file](#gh-body-file), naming it for the insight it carries (`gh-body-insight{index}-{timestamp}.md`), then post it with `gh issue comment {number} --body-file "$body_path"` (ticket number from the session-context manifest), assigning and guarding `body_path` in the same call. Do not inline insight content into the shell command. If no ticket is available, re-route to devlog.
...
Write the artifact when at least one finding became a created ticket in Phase 3 **or** at least one finding was dropped (by being unselected for any ticket-creation action).
