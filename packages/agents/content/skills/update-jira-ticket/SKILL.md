...
2. **Prefer Markdown over ADF.** Reserve `contentFormat: "adf"` for content that Markdown cannot represent faithfully (panels, status lozenges, expand blocks, layout columns). ADF is full-fidelity JSON but verbose and harder to author, so use it only when Markdown genuinely falls short.
...
That is the whole of both non-HTML branches. Those rules exist solely to prevent failures in Jira's HTML→ADF conversion, which is never invoked when you submit ADF or Markdown, so everything under [HTML path](#html-path) is irrelevant on either path, and rendering content to allowlist HTML to run the checker over it is wasted work.
...
The rule classes are: `composition-code-inline-mark`, `named-entity`, `confluence-construct`, `pre-multiline`, `disallowed-element`. The checker may report multiple findings for the same payload; fix them all before the next MCP attempt.
...
`<pre>` is omitted from the allowlist entirely, and the checker also flags multi-line `<pre>` separately. ADF's `codeBlock` node accepts plain text, but the converter mishandles the combination of newlines and quote characters inside the `pre` block. The converter handles the same characters correctly in inline `<code>` inside `<p>`, so the `<pre>` wrapper is the differentiator.
...
`&quot;` and `&apos;` are valid only inside attribute values in which they're needed to avoid clashing with the attribute's quote style. The checker only scans text content for named entities, so legitimate attribute-value uses are not flagged.
...
a. **Probe.** Create a ticket with `<p>ok</p>` as the entire payload. The create call must follow the tagging contract below ([Probe-ticket tagging contract](#probe-ticket-tagging-contract)). If the probe also fails, the problem is call shape, permissions, or the issue itself (not the payload). Stop and report.
...
c. **Cap retries.** Do not exceed 4 retry attempts beyond the original failure. If the bisection has not converged by then, report the smallest failing fragment to the user and stop.
...
A probe ticket that lacks any of these markers will not be matched by the cleanup query below and may stay in the user's backlog indefinitely.
