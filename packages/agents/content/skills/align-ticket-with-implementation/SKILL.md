...
**Ratified-delta mode.** When the caller supplies a delta it has already put to the user, such as a review's proposed-edit preview, the previewed delta is the whole of the revision. Apply exactly its lines and carry every other criterion and section through verbatim. Seek no further divergence between the ticket and the implementation: What the caller's user consented to is those lines, not alignment as such. When applying a line surfaces a change the delta does not contain, stop and report it rather than widening the edit.

Invoked with no delta, the skill aligns to the implementation as described above. The two modes are orthogonal and compose: Criteria-only mode scopes which sections change, ratified-delta mode scopes how much changes within them.
