...
Decompose a story or task into independently orchestrable implementation steps. Each step produces a self-contained task description suitable for `{skill:orchestrate-dev}`. The plan is refined interactively through user feedback.
...
└── {run-id}/                              <- orchestration run directories (created later by {skill:orchestrate-dev})
```
...
- Do not proceed to orchestration — the user invokes `{skill:orchestrate-dev}` separately when ready
