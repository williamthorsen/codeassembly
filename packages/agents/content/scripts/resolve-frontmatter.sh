...
#                             (repeatable). Because items accumulate in the
#                             order given and are never split, an item may
#                             contain `,`.
...
#
# Output (json mode, stdout): A single JSON object. Keys: branch, commit,
...
#
# The script resolves neither `pr` nor `run_id`. Each is populated only when a
# caller passes `--override pr=<url>` or `--override run_id=<id>`.
#
# Exit codes:
#   0  Success.
...
main() {
  ...
  *) fail "unknown --format: $format (expected yaml|json)" ;;
  ...
  # Probe readability before resolving the branch, so that an unreadable repository is named as such.
...
derive_manifest() {
  ...
  # `RESOLVE_FRONTMATTER_BUNDLE_ARGS` passes extra args to the deriver, such as `--home /tmp/xyz` to
  # isolate the global preferences lookup from the real home directory.
  if [[ -n "${RESOLVE_FRONTMATTER_BUNDLE_ARGS:-}" ]]; then
    # shellcheck disable=SC2086 # the args string is word-split on purpose
...
# Optional fields are omitted (rather than emitted as null or empty) so that consumers can rely on `has(field)`
# semantics.
emit_json() {
...
emit_yaml() {
  ...
  for key in "${yaml_extra_keys[@]+"${yaml_extra_keys[@]}"}"; do
    ...
    if [[ "$kind" == "items" ]]; then
      ...
      # An empty flow-list extension emits `key: []`; the omission rule covers scalars alone.
...
# Emits a top-level YAML `key: value` line, auto-quoting the value as needed.
emit_yaml_scalar() {
  local key="$1" value="$2"
  printf '%s: %s\n' "$key" "$(yaml_quote "$value")"
}

# Emits an indented YAML `key: value` line inside the provenance block.
...
# splitting on ASCII US rather than on `,` so that an item containing a comma
# survives whole. Each item is quoted as `emit_yaml_flow_list` quotes its own.
...
# Returns the value either bare or single-quoted, as `needs_yaml_quoting` decides.
...
# so URLs like `https://...` are safe bare. Because `#` is always treated as a comment introducer in YAML 1.1/1.2
# (the spec is permissive about the preceding context), any value containing `#` is quoted.
needs_yaml_quoting() {
  local v="$1"
  [[ -z "$v" ]] && return 0
  [[ "$v" =~ ^[[:space:]] ]] && return 0
  [[ "$v" =~ [[:space:]]$ ]] && return 0
  # A leading `?` is covered by the glyphs-anywhere case below.
  case "$v" in
  -* | :*) return 0 ;;
  esac
  ...
  *'#'* | *'['* | *']'* | *'{'* | *'}'* | \
  ...
  # A value that looks like a YAML boolean, null, or number stays bare: The predicate's value space is
  # the schema's canonical fields and known extensions, in which none of those is ambiguous.
  return 1
}

# Requires each named command to be on PATH. Exits 1 with a clear message when one is missing.
require_commands() {
  local cmd
  for cmd in "$@"; do
...
# Prints an error message to stderr and exits 1.
fail() {
  echo "$PROG: $1" >&2
  exit 1
}
