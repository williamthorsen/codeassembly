...
# caller passes `--override pr=<url>`; PR-aware skills supply the URL that they hold.
#
...
# Marks the artifact as a record rather than a working file. It is emitted into every artifact so that the prohibition
# is present in the file that an agent has open, rather than only in the standing guidance that it may not have loaded.
...
main() {
  ...
  *) fail "unknown --format: $format (expected yaml|json)" ;;
  ...
  # Read `scm`, falling back to the legacy `platform` key so that a pre-rename branch manifest still
...
# strips one) so that a branch like `feat//` produces `feat` in both implementations.
sanitize_branch() {
  local branch="$1"
  branch="${branch//\//-}"
  while [[ "$branch" == *- ]]; do
...
# (the spec is permissive about the preceding context), we quote any value containing `#`.
needs_yaml_quoting() {
  local v="$1"
...
# Used for soft-failure conditions in which the script should continue with degraded behavior (e.g., a duplicate
