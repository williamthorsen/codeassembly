...
A `rejected` fence comes with it, listing one per line the passages that failed, copied from that draft. Revise those passages and nothing else. The bullets outside the fence passed; the caller keeps them and puts your replacements back in their places, so this pass cannot reach them.
