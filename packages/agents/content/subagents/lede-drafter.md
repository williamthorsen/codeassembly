...
- A bullet names the artifact consumed by the reader, backticked: the package, command, flag, file, or rule that the reader reaches for. What the reader consumes decides the marking rather than the kind, so a token that the bullet merely names, such as a heading inside a file or a value passed inside your own pipeline, is quoted instead. A bullet names what that artifact does for the reader, never the internal call that the change edited. An enumeration of the instances touched is not that artifact. Never talk around a name that the reader needs: "An assertion dependency that nothing imported" withholds `@sindresorhus/is`. At `public` tier, define any term that the audience may not share.
