...
5. **Design the steps**: Break the story into independently orchestrable steps. Each step will be executed via `orchestrate-dev` in its own worktree, so it must be fully self-contained.
