...
1. **Render the commit title** using `describe-change.sh` (see `commit-format.md` in the commit skill's `_data/` directory). Pass `--title`, `--scope`, and `--type` and read `commit_title` from the JSON output. If the script is not found, fall back to the bare title.
