...
- **Test comments**: Non-obvious setup not conveyed by the test name; an indirect assertion, naming the reason for the indirection; the rationale for a skip.
...
**Proposed inside a review finding**: A suggested replacement comment is a source comment. This one restates the lookup already expressed by the code and copies an example ID out of the source.
