...
2. **Merge frontmatter** (subagents only). Platform-specific frontmatter overrides are merged into the source's frontmatter from the `_data/{platform}.yaml` of the content root from which the subagent resolved. A source with no overlay contributes no overrides.
