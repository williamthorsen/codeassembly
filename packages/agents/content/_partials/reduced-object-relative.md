...
**Scope.** The rule governs a lexical head noun with an object gap. Two neighboring constructions fall outside it, since the gap is not an object and neither repair applies: the fused head ("everything I know", "all you need") and the adjunct relative ("the reason it fails", "the way it works"). The boundary keys on head type and gap role. Neither a short embedded subject nor a tight word budget exempts anything.
