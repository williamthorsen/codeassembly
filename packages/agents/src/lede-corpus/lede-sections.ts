...
/** Heading of the section containing the lede that the agent published to the pull request. */
export const AGENT_LEDE_HEADING = 'Agent lede';

/** Heading of the section containing the author's critique of the agent's lede. */
export const COMMENT_HEADING = 'Comment';

/** The tag on every lede decision. */
export const LEDE_DECISION_TAG = 'lede-decision';

/** Heading of the section containing the lede that appeared in the merge commit, written only when the two texts differ. */
export const MERGED_LEDE_HEADING = 'Merged lede';

/** A decision record's two ledes and the author's critique of the first, each absent when its section is. */
export interface LedeDecisionPair {
  agentLede: string;
  /** Absent when the author left the agent's lede alone, which marks the edit as null. */
  mergedLede?: string;
  comment?: string;
}

/**
 * Reads the author-approved lede from a decision record's body: the merged lede when the record contains one, the
