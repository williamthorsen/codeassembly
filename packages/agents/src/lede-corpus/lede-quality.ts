// The quality rating of a lede decision. The helper that records a decision and the one that selects exemplars
// from the corpus share this vocabulary and its order, so neither can rename a level or reorder the scale alone.

/** The quality levels that a rated lede may have, ordered lowest to highest. */
...
// to `.includes`, and type assertions are banned. The set's `.has(string)` is the assertion-free lookup.
 ...
 * tuple's order is the scale, so a level's rank is its position in the tuple rather than any property of the level
 * itself.
 */
