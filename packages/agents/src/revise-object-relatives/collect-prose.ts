...
/**
 * Builds the generator markers for one file kind, each held to a line opening with `lead` so a document naming a
 * marker in prose is still read. `@generated` may sit anywhere on that line, since a generator pairs it with
 * directives of its own, and the phrase pair matches without regard to case, which reaches the shouted "DO NOT EDIT."
 * form. Both phrases are required: A file bounding a generated region announces it as "Generated by" alone, and the
 * prose around that region is authored.
 */
function buildGeneratorMarkers(lead: string): readonly RegExp[] {
  const anchor = String.raw`^[ \t]*${lead}[ \t]*`;
  return [
    new RegExp(String.raw`${anchor}.*@generated\b`, 'm'),
    new RegExp(String.raw`${anchor}.*\bgenerated by\b.*\bdo not edit\b`, 'im'),
  ];
}
