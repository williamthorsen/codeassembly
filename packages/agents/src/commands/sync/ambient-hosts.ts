...
/** Why one ambient host is skipped. A stale install includes the probe status that names its remedy. */
 ...
 * existing region). Both domains share this one path, differing only in the host that each targets and in who owns
 * region creation there.
 */
...
/**
 * Pairs each probed host with the plan that a sync carries out there, derived once so that both paths read the same
 * one.
 */
...
/** One targeted ambient host paired with the plan that a sync carries out there. */
 ...
 * each paired with the harness id for which its content is rendered. Read once here rather than per consumer, so that
 * the damaged-region gate and the delivery plan cannot disagree about what a host holds.
 */
...
/** One targeted ambient host, the harness that its content renders for, and the state found there by a probe. */
...
/**
 * The line with which an ambient region opens, so that a reader who finds generated guidance knows an edit there is
 * lost.
 */
...
function planAmbientHost(hostKind: AmbientHostKind, host: AmbientHostState, body: string): AmbientHostPlan {
  ...
  // A damaged region is never written: `findDamagedAmbientHostDefects` has already failed the run.
