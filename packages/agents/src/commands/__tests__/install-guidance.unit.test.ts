...
describe('guidance installation', () => {
  ...
  describe('retired shared guidance', () => {
    ...
    it('removes a tracked copy deployed by a previous version', async () => {
      await setupClaudeHome();
    ...
    it('preserves a tracked copy containing hand-written content, and stops tracking it', async () => {
      await setupClaudeHome();
    ...
    it('leaves untouched a copy that this CLI never deployed', async () => {
      await setupClaudeHome();
  ...
  describe('retired shared guidance on uninstall', () => {
    ...
    it('preserves a tracked copy containing hand-written content', async () => {
      await setupClaudeHome();
      const handWritten = '# Retired\n\nMy own notes.\n';
      const retiredPath = await seedRetiredSharedGuidance({ contentOnDisk: handWritten });
    ...
    it('removes a tracked copy containing hand-written content with --force', async () => {
      await setupClaudeHome();
  ...
  describe('ambient region preservation', () => {
    ...
    /** The whole region content written by a `sync --global` run: the generated note above the rulebook blocks. */
  ...
  describe('include directive expansion errors', () => {
    /**
     * Builds the minimum content tree that the install pipeline expects to traverse, so that a test can inject a
  ...
  describe('unresolvable in-body anchors', () => {
    ...
    // Shared guidance is inlined into a harness file by an include directive, so its dead anchor is reported against
    // the file that inlines it: The shared source is no longer checked on a route of its own.
