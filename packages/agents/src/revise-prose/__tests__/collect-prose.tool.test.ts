...
const FIXTURE_FILES: Readonly<Record<string, string>> = {
  ...
  'sealed.md': '<!-- Sealed record. Do not edit this file. -->\n\nSealed prose the sweep reads.\n',
  'src/helper.ts': '/** Authored doc prose the sweep reads. */\nexport const helper = 1;\n',
  'vendor-marker-literal.ts':
    "/** Quoting doc prose the sweep reads. */\nexport const marker = '<!-- Extracted verbatim from a plugin. Do not edit. -->';\n",
  'vendor-marker-prose.md':
    'A comment reading "Extracted verbatim from a plugin. Do not edit." marks an extract.\n\nMentioned prose the sweep reads.\n',
  'vendored.md':
    '---\nname: vendored\n---\n\n<!-- Extracted verbatim from a plugin. Do not edit. -->\n\nVendored prose the sweep may not edit.\n',
  'vendored.sh':
    '#!/usr/bin/env bash\n# Extracted verbatim from a plugin. Do not edit.\n# Vendored shell prose the sweep may not edit.\n',
  'vendored.ts':
    '// Extracted verbatim from a plugin. Do not edit.\n/** Vendored doc prose the sweep may not edit. */\nexport const vendored = 1;\n',
};
...
describe(collectProse, () => {
  ...
  it('counts every excluded file under the generated reason', async () => {
    expect((await sweep()).skipped.generated).toBe(6);
  });

  it('yields nothing from a verbatim extract, whether its marker follows frontmatter, a shebang, or nothing', async () => {
    const text = joinText(await sweep());

    expect(text).not.toContain('Vendored prose');
    expect(text).not.toContain('Vendored shell prose');
    expect(text).not.toContain('Vendored doc prose');
  });

  it('counts every verbatim extract under the vendored reason', async () => {
    expect((await sweep()).skipped.vendored).toBe(3);
  });

  it('reads a file that says "Do not edit" alone or names the extract marker outside a comment line', async () => {
    const text = joinText(await sweep());

    expect(text).toContain('Sealed prose the sweep reads.');
    expect(text).toContain('Mentioned prose the sweep reads.');
    expect(text).toContain('Quoting doc prose the sweep reads.');
  });
  ...
  it('leaves a YAML file carrying only data out of the scanned set, so it reaches no batch', async () => {
    ...
    expect(collection.scannedFiles.map((scanned) => scanned.file)).not.toContain('data.yaml');
  });

  it('yields nothing from the sweep record, which the helper writes', async () => {
    const collection = await sweep();

    expect(collection.files).not.toContain('.agents/revise-prose.yaml');
