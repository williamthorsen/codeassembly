...
/**
 * Derives what a branch of commits adds up to: the entries it declares, the head they consolidate to, the commits no
 ...
 * the head its trailers already consolidate to, so reading both would count the branch against itself.
 *
 * A subject no template matches is reported rather than dropped, so a mistyped prefix is visible to its author instead
 * of silently shrinking the set the head is derived from. A violation likewise leaves its entry untouched: the commit
 * is already written, and refusing here would block the pull request behind a rebase.
 *
 * A branch with no entries yields no head. An empty record would be indistinguishable from a head that names no scope.
 */
export function classifyCommits(
  ...
  const unclassified: UnclassifiedCommit[] = [];
  ...
  for (const commit of commits) {
    ...
    for (const subject of subjects) {
      ...
      if (record === undefined) {
        unclassified.push({ commit: commit.hash, subject });
        continue;
      }
  ...
  const head =
    entries.length > 0
      ...
      : undefined;
  return { entries, unclassified, violations, ...(head !== undefined && { head }) };
}

/** What a branch of commits adds up to. `head` is absent where no entry was found to derive one from. */
export interface Classification {
  entries: ClassifiedEntry[];
  head?: ChangeRecord;
  unclassified: UnclassifiedCommit[];
...
export interface UnclassifiedCommit {
  commit: string;
  subject: string;
}
