...
describe(makeArtifactMarker, () => {
  describe.each(['skill', 'subagent'] as const)('type=%s', (type) => {
    const marker = makeArtifactMarker(type);
    const otherType = type === 'skill' ? 'subagent' : 'skill';
    ...
    it("returns undefined for the other type's marker, keeping the namespaces disjoint", () => {
      const otherMarked = makeArtifactMarker(otherType).injectMarker(FILE, 'people-report');
    ...
    it('inserts the marker immediately after the frontmatter block', () => {
      ...
      expect(output).toBe(
        `---\nname: people-report\nuser-invocable: true\n---\n<!-- codeassembly-${type}:people-report -->\n\n# People report\n\nBody.\n`,
      );
    });
