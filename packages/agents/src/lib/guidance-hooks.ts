 ...
 * moving between the two spellings has nothing but this to tell them which one they wrote. The trailing group admits a
 * payload only behind a colon, which keeps prose that merely mentions a guidance hook from reading as an attempt to
 * declare one.
 */
const HOOK_LIKE_REGEX = /^[ \t]*<!--[ \t]*guidance[ \t-]?hooks?[ \t]*(?::[^>]*)?-->[ \t]*$/i;

/** The kebab-case, letter-led slug grammar that a hook name must satisfy, matching every other slug in the system. */
const HOOK_NAME_REGEX = /^[a-z][a-z0-9-]*$/;

/** Reason a body's guidance-hook declarations were rejected, reported in error messages. */
type FailureReason = 'duplicate-hook' | 'fill-in-frontmatter' | 'malformed-name' | 'unrecognized-directive';

/**
 * A body with its guidance hooks resolved. `content` contains the fills; `stripped` is the same body with every
 ...
 * `stripped` separates a collision introduced by a binding from one that the host had all along.
 */
...
/** A guidance-hook declaration in a body: the slot name and the 1-based line that its directive occupies. */
...
/**
 * One rulebook bound to a guidance hook: the slug to which its block is attributed, its already-rendered body, and the
 * version named in its block when it declares one.
 */
  ...
  try {
    ...
    throw new Error(
      `${sourceLabel} resolves its own anchors and stops resolving once a guidance-hook fill is spliced in, so a ` +
        ...
        `it to a hook that this body does not declare.\n${describeError(error)}`,
      { cause: error },
    );
  }
}
...
/**
 * Collects every guidance hook that `body` declares, in source order. Throws `GuidanceHookError` for a name outside the slug
 ...
 * begin with the `codeassembly-` prefix that identifies generated content, which also keeps them clear of the
 * directive grammar against which a source body is parsed.
 */
