...
/** Returns the slug stamped in a skill file's sync ownership marker, or `undefined` when it contains none. */
 ...
 * (`name`, `description`, `user-invocable`), the sync ownership marker, the version line when the rulebook declares a
 * version, and the rulebook's neutral body. The frontmatter `name` is the resolved skill name (the display
 * label); the ownership marker records the stable `slug`, so retraction can recover a rulebook's identity even when
