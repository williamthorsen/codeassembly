...
/** The line the `prompts.yml` region opens with, so a reader knows an entry added inside it is lost on the next sync. */
const promptsRegionNote =
 ...
 * items — no `prompts:` header — single-quoting descriptions with internal quotes doubled. This is the body the region
 * renderer wraps in sentinels. No entries yields an empty string rather than a note-only body: the caller reads
...
export function renderPromptEntries(entries: ReadonlyArray<PromptEntry>): string {
  ...
  return yamlLines.length === 0 ? '' : [promptsRegionNote, ...yamlLines].join('\n') + '\n';
}
