...
/**
 * The routes that guidance takes into a session. `ambient` and `skill` instruct the resolver, which inlines an ambient
 ...
 * Declaring it records the route by which the rulebook is reached, which lets a mismatch between the two be named.
 */
const DeliveryModeSchema = z.enum(['ambient', 'hook', 'skill']);

/**
 * The rejection reported for every malformed `delivery`, whichever branch failed. Named once and attached to both,
 * because a union reports its own failure for a value that neither member accepts but reports a member's issue
 * verbatim when only that member applies, which an empty list does.
 */
const DELIVERY_ERROR = "delivery must be 'ambient', 'hook', or 'skill', or a non-empty list of them";

/** The rejection reported for a non-string `version`: The schema received it as a number, so quoting is the fix. */
