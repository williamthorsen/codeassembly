...
/** `git check-ignore`'s exit status for a path that it does not ignore, as distinct from a failure to answer at all. */
...
export async function checkGitIgnored(cwd: string, filePath: string): Promise<boolean | undefined> {
  try {
    ...
    // answer -- reported as `undefined` so that a caller does not read it as "not ignored".
