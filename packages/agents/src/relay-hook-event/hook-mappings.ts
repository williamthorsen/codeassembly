...
const HOOK_MAPPINGS: Readonly<Record<HarnessId, Readonly<Record<string, HookMapping>>>> = {
  ...
  rovo: {
    // `on_session_end` covers exit, session switch, and fork alike, so what distinguishes them sits inside
    // `attributes`.
...
/** Names every harness the relay serves. */
export function listRelayHarnesses(): readonly string[] {
  return Object.keys(HOOK_MAPPINGS);
}

/**
 * The hook names the relay serves for `harness`, in table order. Configured hook entries compose from this list, so
 * they cannot drift from the hooks that the relay answers.
 */
...
/** True when `value` names a harness the relay serves. */
