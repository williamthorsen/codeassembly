...
# Exports the project root for Include directives and isolates the run from the user's git configuration.
...
# the agent sandbox denies. The X run stays at the end, the only position that macOS substitutes.
make_tmpdir() {
