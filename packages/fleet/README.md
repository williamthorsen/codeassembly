...
Alongside the event fold, a git adapter polls each lane's worktree (the `cwd` that its events report) with read-only git commands and overlays the results on the lane: checked-out branch, working-tree change count, ahead/behind against the base branch (`behind > 0` is base-branch advance), and the base ref compared against. Observations are kept in memory and merged at snapshot time, never written to the event store, and the adapter never fetches or otherwise mutates a repository, so base-branch advance is measured against the local remote-tracking ref, which worktrees share.
...
| `FLEET_STALE_MS`      | `90000`                  | Quiet threshold after which a working session is reported as stale.                         |

Empty or non-numeric values fall back to the defaults.

Filesystem watching is best-effort: When recursive `fs.watch` is unavailable, the server announces rescan-only mode at startup and relies on the rescan interval for correctness.
